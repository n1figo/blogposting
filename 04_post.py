...
def read_and_filter_excel(file_path):
    ...
    df_filtered = df[df['Contract End Date'].dt.month.isin([previous_month, this_month, next_month])]

    # Calculate booking available date
    df_filtered['Available Booking Date'] = df_filtered['Contract End Date'].apply(lambda x: np.busday_offset(x, 4))
